...
type Props = {
  ...
  disabled?: boolean;
  layout?: "auto" | "sidebar" | "bar" | "mobile";
};
...
function pillClasses(isActive: boolean, disabled: boolean) {
  return [
    "type-ui shrink-0 whitespace-nowrap rounded-full px-4 py-2 tracking-wide transition-all duration-200",
    disabled
...
function sidebarClasses(isActive: boolean, disabled: boolean) {
  return [
    "type-ui w-full rounded-xl px-3 py-2.5 text-left text-sm leading-snug transition-colors",
    disabled ? "cursor-not-allowed opacity-60" : "hover:bg-cream",
    isActive ? "bg-espresso text-cream shadow-soft" : "text-espresso/80",
  ].join(" ");
}
...
export default function CategoryTabs({
  ...
  const sheet = open
    ? createPortal(
        ...
        >
          ...
          <div className="animate-sheet-up relative flex max-h-[min(85dvh,28rem)] w-full max-w-lg flex-col rounded-t-3xl border border-gold/15 bg-parchment pb-[env(safe-area-inset-bottom,0px)] shadow-2xl sm:max-h-[min(70vh,32rem)] sm:rounded-3xl">
            <div className="flex items-center justify-between border-b border-gold/15 px-4 py-4">
              ...
              <button
                ...
                onClick={() => setOpen(false)}
                aria-label={tUi("close", lang)}
                className="touch-target touch-manipulation flex items-center justify-center rounded-full text-warmgray transition hover:bg-cream hover:text-espresso"
              >
                <X className="h-5 w-5" aria-hidden="true" />
              </button>
            </div>

            <ul className="no-scrollbar flex-1 overflow-y-auto px-3 py-2 pb-[max(1rem,env(safe-area-inset-bottom))]">
              {tabs.map((tab) => {
  ...
  const showSidebar = layout === "sidebar" || layout === "auto";
  const showBar = layout === "bar" || layout === "auto";
  ...
  return (
    <>
      {showSidebar && (
        <nav
          aria-label={tUi("categories", lang)}
          className={layout === "auto" ? "hidden lg:block" : undefined}
        >
              ...
              return (
                <li key={tab.id}>
                  <button
                    ...
                    disabled={disabled}
                    onClick={() => handleSelect(tab.id)}
                    className={sidebarClasses(isActive, disabled)}
      ...
      {showBar && (
        <nav
          aria-label={tUi("categories", lang)}
          className={layout === "auto" ? "hidden md:block lg:hidden" : undefined}
        >
          <ul className="no-scrollbar -mx-1 flex gap-2 overflow-x-auto px-1 pb-0.5">
            {tabs.map((tab) => {
              const isActive = tab.id === active;
              return (
      ...
      {showMobile && (
        <nav
          aria-label={tUi("categories", lang)}
          className={layout === "auto" ? "md:hidden" : undefined}
        >
          <button
            type="button"
            disabled={disabled}
