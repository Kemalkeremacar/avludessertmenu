...
export default function WelcomeOverlay({ cafeName, categories, onSelect }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex animate-fade-in">
      ...
      <div className="relative flex h-[100dvh] w-full items-stretch justify-center px-4 pb-[max(1rem,env(safe-area-inset-bottom))] pt-[max(1.5rem,env(safe-area-inset-top))] sm:px-6 md:items-center md:p-8">
        <div className="flex min-h-0 w-full max-w-md flex-col md:max-h-[min(90vh,44rem)] md:max-w-3xl md:rounded-3xl md:border md:border-cream/15 md:bg-espresso/50 md:p-8 md:shadow-2xl md:backdrop-blur-md lg:max-w-4xl lg:p-10">
          ...
          <div className="no-scrollbar mt-4 min-h-0 flex-1 overflow-y-auto sm:mt-6">
            <div className="grid grid-cols-2 gap-2.5 pb-2 sm:gap-3 md:flex md:flex-wrap md:justify-center md:gap-3">
              {categories.map((c, i) => (
                <button
                  key={c.id}
                  type="button"
                  ...
                  className="group touch-manipulation animate-fade-in flex min-h-[5.75rem] w-full flex-col items-center justify-center gap-2 rounded-2xl border border-cream/15 bg-cream/[0.07] px-2 py-3 text-center backdrop-blur-md transition-all duration-200 hover:border-gold/40 hover:bg-cream/[0.12] active:scale-[0.98] active:border-gold/50 active:bg-cream/[0.14] sm:min-h-[6.5rem] sm:gap-2.5 sm:px-3 sm:py-4 md:h-[7.25rem] md:w-36 md:shrink-0 md:px-2 lg:w-[9.5rem]"
                >
